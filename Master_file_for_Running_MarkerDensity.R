...
this.heritabilities.vector <- 0.7
...
the.taxa.selected.from.MAS <- identify.top.performing.lines.via.MAS(the.phenotypes = pheno_simulated, the.genotypes = geno_imp.non.imp, 
...
for(z in 1:length(unique(names(the.taxa.selected.from.GS)))){
  ...
  the.backcross.population.inds.selected.from.GS <- create.backcross.population(the.genotypic.data = genotypes.combine.genotypes, the.individuals.of.interest = the.taxa.selected.from.GS_z, 
                                                                              the.parent = "Kaskade.Justin") #Kaskade.Justin these.pheno.and.geno.data$genotypes
...
the.backcross.population.inds.selected.from.MAS <- create.backcross.population(the.genotypic.data = genotypes.combine.genotypes, the.individuals.of.interest = the.taxa.selected.from.MAS, 
                                                                              the.parent = "Kaskade.Justin") #Kaskade.Justin
...
for(zz in 1:length(model_names)){
  ...
  the.backcross.population.inds.selected.from.GS <- the.backcross.population.inds.selected.from.GS[match(ggnames, rownames(the.backcross.population.inds.selected.from.GS)),]
...
the.backcross.population.inds.selected.from.MAS <- the.backcross.population.inds.selected.from.MAS[match(ggnames, rownames(the.backcross.population.inds.selected.from.MAS)),]
...
the.backcross.population.inds.selected.from.GS_zzz <- the.backcross.population.inds.selected.from.GS_zzz[match(ppnames, rownames(the.backcross.population.inds.selected.from.GS_zzz)),]
#Fit rrBLUP model in the current generation and validate in the GS selected backcross population # polyrad genomic data doesn't work in the place of these.pheno.and.geno.data$genotypes here
performance.of.GS.in.GS.BC1 <- rrBLUP.current.generation.validate.in.BC.generation(the.phenotypes.current.gen = pheno_simulated, the.genotypes.current.gen = geno_imp.non.imp,
...
str(performance.of.GS.in.GS.BC1)
names(performance.of.GS.in.GS.BC1)
the.backcross.population.inds.selected.from.MAS <- the.backcross.population.inds.selected.from.MAS[match(ggnames, rownames(the.backcross.population.inds.selected.from.MAS)),]
#Fit rrBLUP model in the current generation and validate in the MAS selected backcross population
performance.of.GS.in.MAS.BC1 <- rrBLUP.current.generation.validate.in.BC.generation(the.phenotypes.current.gen = pheno_simulated, the.genotypes.current.gen = these.pheno.and.geno.data$genotypes,
                                                                                   the.genotypes.BC = the.backcross.population.inds.selected.from.MAS, the.phenotypes.BC = phenotypes.in.MAS.backcross.population,
                                                                                   the.output.dir = this.output.dir, GS.or.MAS = "MAS")


setwd(home.dir)

str(performance.of.GS.in.MAS.BC1)

#Fit rrBLUP model in the current generation and validate in the MAS selected backcross population
performance.of.MAS.in.MAS.BC1 <- GWAS.current.generation.validate.MAS.in.BC.generation(the.phenotypes.current.gen = pheno_simulated,
...
setwd(home.dir)
#Fit rrBLUP model in the current generation and validate in the MAS selected backcross population
performance.of.MAS.in.GS.BC1 <- GWAS.current.generation.validate.MAS.in.BC.generation(the.phenotypes.current.gen = pheno_simulated,
                                                                                       the.genotypes.current.gen = these.pheno.and.geno.data$genotypes,
                                                                                       the.genotypes.BC = the.backcross.population.inds.selected.from.GS,
                                                                                       the.phenotypes.BC = phenotypes.in.GS.backcross.population,
                                                                                       the.output.dir = this.output.dir, GS.or.MAS = "GS",
                                                                                       number.of.markers.to.use.for.MAS = 4)
